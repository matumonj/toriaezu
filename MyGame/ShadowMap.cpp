...
HRESULT ShadowMap::Initialize(ID3D12Device*device)
{
	HRESULT hr{};
...
void ShadowMap::CreateShadowBuffer(ID3D12Device* device)
{
	HRESULT result;

	D3D12_DESCRIPTOR_HEAP_DESC shadow_descriptor_heap_desc{};
	shadow_descriptor_heap_desc.NumDescriptors = 1;
	shadow_descriptor_heap_desc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_DSV;
	shadow_descriptor_heap_desc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_NONE;
	shadow_descriptor_heap_desc.NodeMask = 0;
	result = device->CreateDescriptorHeap(&shadow_descriptor_heap_desc, IID_PPV_ARGS(&dh_shadow_buffer_));
