...
void FbxLoader::ParseMeshVertices(f_Model* model, FbxMesh* fbxMesh)
{
    ...
    for (int i = 0; i < controlPointsCount; i++) {
        ...
        if (fbxMesh->GetPolygonVertexNormals( normal)) {
            vertex.normal.x = (float)normal[i][0];
            vertex.normal.y = (float)normal[i][1];
            vertex.normal.z = (float)normal[i][2];
        }
        if (textureUVCount > 0) {
            if (fbxMesh->GetPolygonVertexUVs(uvNames[0], uvs)) {
                vertex.uv.x = (float)normal[i][0];
                vertex.uv.y = (float)normal[i][1];
                //vertex.uv.z = (float)normal[i][2];
            }
        }
